...
const AkademiDersProgrami = () => {
  const [schedule, setSchedule] = useState([]);
  const [loading, setLoading] = useState(true);

  const gunler = ['Pazartesi', 'Salı', 'Çarşamba', 'Perşembe', 'Cuma'];
  const saatler = ['09:00', '10:00', '11:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00'];
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/api/academic/ders-programi');
...
export default AkademiDersProgrami; 